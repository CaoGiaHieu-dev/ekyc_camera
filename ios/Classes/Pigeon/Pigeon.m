  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:CameraInterfaceGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(takePhotoWithCompletion:)], @"CameraInterface api (%@) doesn't respond to @selector(takePhotoWithCompletion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        [api takePhotoWithCompletion:^(FlutterStandardTypedData *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    }
